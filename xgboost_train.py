...
if __name__ == "__main__":
    ...
    train = data_train.drop('is_canceled', axis=1)
    label_train = pd.DataFrame(data_train['is_canceled'])
    dtrain = xgb.DMatrix(train, label=label_train)
    
    
    data_validation = pd.read_csv(f"{args.validation_data_dir}/validation.csv")
    validation = data_validation.drop('is_canceled', axis=1)
    label_validation = pd.DataFrame(data_validation['is_canceled'])
